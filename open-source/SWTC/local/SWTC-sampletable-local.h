...
#ifndef setting 
#include "../../Common/local/local-setting.h"
#define setting
#endif
using namespace std;
...
public:
	...
	void modify_triangle(sample_node* pos_s, sample_node* pos_d, int op) //op = 1 means to add, op = -1 means to minus.
	{
		...
		for(int i=0;i<common_neighbor.size();i++)
		{
			...
			int sum = f1*f2*op;
			node_table->ID_to_pos(common_neighbor[i])->local_cnt += sum;
			trcount += sum;
			pos_s->local_cnt += sum;
			pos_d->local_cnt += sum;
		}
	...
	void insert(unsigned int s_num, unsigned int d_num, int g, double p, long long time, long long land_mark, long long last_mark)
	{
		unsigned int pos = (rand() % group_size) + group_size*g; // in case that the range of rand() is not sufficient.
