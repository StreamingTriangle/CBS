...
public:
	int size;
	int tsl_head, tsl_tail; // mark the head and the tail of the list. expiration is only used in BPS-sample, namely the baseline method. As in this method there are two kinds
	...
	void replace_sample(unsigned int s_num, unsigned int d_num, double p, long long time, bool bi_drection, int pos) // replace the sample edge at pos with a new edge; 
	{
		...
		table[pos].reset(s_num, d_num, p, time, tsl_tail, -1, bi_drection); // insert the new edge;
	...
	void replace_vice(unsigned int s_num, unsigned int d_num, double p, long long time, bool bi_direction, int pos) // replace the vice edge at pos with a new edge;
	{
		...
		table[pos].vice.reset(s_num, d_num, p, time, tsl_tail, -1, bi_direction); // insert the new edge;
		set_time_list(tsl_tail, 1, tsl_pos);
		tsl_tail = tsl_pos;
		if (tsl_head == -1)
			tsl_head = tsl_pos;
