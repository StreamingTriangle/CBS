...
public:
		...
		sample_node* insert(unsigned int node, int out_edge_ = -1, int in_edge_=-1, int bi_edge = -1);
...
sample_node* NodeTable::insert(unsigned int node, int out_edge_, int in_edge_, int bi_edge_)
{
	...
	for (int i = 0; i<k; i++)
	{
		...
		{
			table[address] = sample_node(node, out_edge_, in_edge_, bi_edge_);
		...
		if (table[address].nodeID == node)
		{
			...
			if(bi_edge_>=0)
				table[address].set_edge(bi_edge_, bi);
			return &(table[address]);
		}
		else
		{
			...
			while (tmp)
			{
				...
				{
					if (out_edge_>=0)
						tmp->set_edge(out_edge_, out);
					if(in_edge_>=0)
						tmp->set_edge(in_edge_, in);
					if (bi_edge_ >= 0)
						tmp->set_edge(bi_edge_, bi);
					return tmp;
				}
				tmp = tmp->next;
			}
	...
	sample_node* tmp = new sample_node(node, out_edge_, in_edge_, bi_edge_);
