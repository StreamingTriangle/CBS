...
public:
		...
		sample_node* insert(unsigned int node, int edge = -1);

		sample_node* set_edge(unsigned int node, int edge);

		int get_edge(unsigned int node);
...
sample_node* NodeTable::insert(unsigned int node, int edge)
{
...
sample_node* NodeTable::set_edge(unsigned int node, int edge)
{
	string node_s = my_to_string(node);
	for (int i = 0; i<k; i++)
	{
...
int NodeTable::get_edge(unsigned int node)
{
	string node_s = my_to_string(node);
	for (int i = 0; i<k; i++)
	{
