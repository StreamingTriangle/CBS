...
int main()
{
	...
	ifstream fin("./golden-" + my_to_string(direction) +"-direct-count.txt");
	...
	fin.close();
	string prefix = "./";
	string postfix = "-direct-count.txt";
	double error_sum = 0;
	double avg_max =0;
	for(int i=0;i<round;i++)
	{
		...
		double max_error = 0;
		ifstream fin((prefix + algorithm_name + "-" + my_to_string(i) + "-"+ my_to_string(direction) + postfix).c_str());
		unsigned int cnt = 0;
		unsigned int sum = 0;
		while(fin>>trcount)
		{
