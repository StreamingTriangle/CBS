...
void Graph::local_edge_count(unordered_map<unsigned long long, int> &cr)
{
	...
	for (it = g.begin(); it != g.end(); it++)
	{
		...
		for (int j = 0; j < v1.size(); j++)
		{
			...
			if (it2 == g.end())
			{
				unsigned long long e3 = (((unsigned long long)src<<32)|anc);
				if(cr.find(e3)==cr.end())
					cr[e3] = 0;	
				continue;
			}
			else
			...
			while (cur)
			{
				...
				{
					sum++;
					unsigned long long e1 = (((unsigned long long)src<<32)|cur->Snode); 
					if(cr.find(e1)!=cr.end())
						cr[e1]++;
					else
						cr[e1] = 1;
					unsigned long long e2 = (((unsigned long long)anc<<32)|cur->Snode);
					if(cr.find(e2)!=cr.end())
						cr[e2]++;
					else
						cr[e2] = 1;
				}
	...
	if(cr.size()!=edgecount)
	{
		cout<<"edge missed! "<<cr.size()<<' '<<edgecount<<endl;		
	}
	return;
}
